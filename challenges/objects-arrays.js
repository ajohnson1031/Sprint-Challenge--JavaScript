...
//const universities = []; BUMP!
const universities = graduates.map(elem => {
  return elem.university;
});
...
//const contactInfo = []; BUMP!
const contactInfo = graduates.map(elem => {
  return `${elem.first_name}  ${elem.email}`;
});
...
const uni = [];
const calcUni = graduates.forEach(elem => {
  if (elem.university.includes("Uni")) uni.push(elem.university);
});

console.log(uni.length); //Find out how many universities have the string "Uni" included in their name.
...
*/
const animalNames = [];
const getNames = zooAnimals.forEach(elem => {
  animalNames.push(
...
*/
//const lowerCase = []; BUMP!
const lowerCase = zooAnimals.map(elem => {
...
const getLowerPopulation = zooAnimals.filter(elem => {
  elem.population < 5 ? lowerPopulation.push(elem) : null;
});
...
*/
const popArray = [];
const pushPop = zooAnimals.forEach(elem => {
  popArray.push(elem.population);
});

const populationTotal = popArray.reduce((acc, curr) => acc + curr);
